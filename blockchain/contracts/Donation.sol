...
contract DonationContract {
    ...
    modifier notEmergencyStopped() {
        require(
            !emergencyStop,
            "Operations for this organisation are currently paused"
        );
        _;
    }
    ...
    function donate(
        ...
        uint organisationId
    ) public payable notEmergencyStopped {
    ...
    function withdrawFunds(
        ...
        require(
            organisations[organisationId].amountReceived > 0,
            "No funds to withdraw or not a beneficiary"
        );
        require(
            organisations[organisationId].requests[msg.sender] == address(0),
        ...
        for (
            uint i = 0;
            i < organisations[organisationId].signers.length;
            i++
        ) {
            if (
                organisations[organisationId].requests[msg.sender] == _toAddress
            ) {
                signatureCount++;
            }
        }
        ...
        require(
            signatureCount > organisations[organisationId].signersRequired,
            "Not enough signatures"
        );

        // transfer funds
        payable(_toAddress).transfer(
            organisations[organisationId].amountReceived
        );
        emit FundsWithdrawn(
            _toAddress,
            organisations[organisationId].amountReceived,
